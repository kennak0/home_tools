...
- **アプリ内の「QR コードを読み取る」** → カメラ映像を jsQR（`vendor/jsqr`、端末内）で読む。
  ...
  URL でも合言葉そのものでも受ける
- **`getUserMedia` はタップの直後に呼ぶ**（`login.js`）。`await import()` や jsQR の読み込み（256KB）を
  挟んでから呼ぶと、iOS では user activation が切れて**許可ダイアログが出ないまま何も起きない**。
  jsQR の読み込みはカメラの許可待ちと並行させる。15 秒で応答が無ければ写真経路に切り替える
- カメラが使えなかった理由（権限拒否・カメラ無し・他アプリが使用中・タイムアウト）は必ず画面に出す。
  「押しても何も起きない」状態を作らない
- **許可が下りても映像が来ないブラウザがある**。iOS の Orion で確認（2026-09-22。Safari は問題なし）。
  `getUserMedia` は成功するのに `video.videoWidth` が 0 のままなので、8 秒で打ち切って写真経路に
  切り替える（`qr-scan.js` の stall 検出）。`video.play()` は **await しない**。映像が来ないと
  解決も reject もせず、待つと stall 判定まで進めないまま「開いたのに何も起きない」になる
...
- **表形式の行**もこの形で受ける: 行頭が `月 | 日 | 曜日` か `日 | 曜日`（区切りは空白か、OCR が
  罫線を写した `|`）。月は行にあればそれを使い、無ければ見出しから引き継ぐ。部活動予定表のように
  1 行目だけ月が入る表を想定している
- 見出しの月は行の途中にあっても拾う（`R8 サッカー部 9月 部活動予定表`）。ただし**その行に日付か
  時刻があれば見出しにしない**（`9月 1日 運動会` は予定であって見出しではない）
...
#### 実測: 実物の「部活動予定表」はこの構成では読めない（2026-09-22）

A4 のプリント（縦型の表。列は 月 / 日 / 曜日 / 学校行事等 / 場所 / 練習時間 / 半面・全面）を
iPhone で撮った写真 1 枚で、手元の Firefox を使って測った。条件を変えた比較は次のとおり。

| 条件 | 信頼度 | 取れた文字数 | parse した件数 |
|---|---|---|---|
| 現行（fast 辞書 / PSM 4 / 長辺 2600px） | 60 | 987 | 10（ほぼ誤り） |
| best 辞書（13MB）に差し替え | 61 | 1091 | 12（ほぼ誤り） |
| 表の部分だけ切り出して拡大 | 54 | 193 | 0 |
| PSM 6（1 ブロック） | 41 | 792 | 0 |
| PSM 11（疎なテキスト） | 66 | 1482 | 15（ほぼ誤り） |

分かったこと。

- **best 辞書はほぼ効かない**（信頼度 60 → 61）のに時間が 2.3 倍（9 秒 → 21 秒）、
  配信も +13MB。**採用しない**
- **切り出して拡大すると却って悪化する**。「大きく写せば読める」は成り立たない
- **PSM 4 と PSM 11 は失い方が違う**。4 は行の並びを保つがセルの中身が崩れ、
  11 はセルの中身をよく読むが行の構造を失う（`16 : 15 ~ 18 : 30` や
  `新人戦 予選(会場:大森一) vs大森ハ12:00` は 11 なら読めている）
- PSM 11 の**単語の bbox（948 語）を y 座標で束ね直すと、表の行は復元できる**。
  Tesseract の行組み立てに頼らなければ構造は取り戻せる
- 残る壁は**数字**。罫線と混ざって **「1」が落ちる**のが系統的に起きる
  （`16:00~18:30` → `6 … 00 … 8 … 30`、`10日` → `0`、`11日` → `1`）。
  予定表は日付と時刻が命なので、これが直らない限り実用にならない

つまり **2b を使いものにするには「bbox から表を組み直す」＋「数字の復元」の 2 段が要る**。
一方 2a（iOS の写真アプリの Live Text でテキストを選んでコピー）は Apple の OCR を使うので、
この手の印刷物では 2b より精度が高いと見込んでいる（比較表のとおり）。**まず 2a で同じ紙を
試し、それで足りるなら 2b は「テキストを選べない画像」向けの保険に留める**。
